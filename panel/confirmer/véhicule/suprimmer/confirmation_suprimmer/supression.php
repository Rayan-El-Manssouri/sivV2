...
<head>
    ...
    <link href="https://fonts.googleapis.com/css2?family=Cormorant+SC:wght@300&family=Montserrat:wght@200;400&display=swap" rel="stylesheet"> 
...
<header>
    ...
    <div class="suppression">
        ...
        <?php foreach($data as $dataV2): ?>
            ...
            <div class="Bouton">
                <form method="POST">
                    <input  name="test" type="submit" value="Supprimer" id="test" onclick="return confirm('Etes vous sur de vouloir le supprimmer ?');"></input>
                </form>
                <a href="../suprimmer.php?id=<?= $_SESSION['id']?>">
                    <button >Revenir a la page de suppression</button>
                </a>
            </div>
        <?php endforeach; ?>
        ...
        <?php 
        if(isset($_POST['test'])){
            $query1 = "DELETE FROM vehicule WHERE IdVehicule='".$_GET['id']."' " ;
            $data1= $database->read($query1);
            $id = $_SESSION['id'];
            ?>
            <script>
                location.replace("../suprimmer.php?id=<?=$id?> ")
            </script>
            <?php
            die();
        }
