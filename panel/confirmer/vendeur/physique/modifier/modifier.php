...
<head>
    ...
    error_reporting(0);
        ...
        $query = "SELECT * FROM `vendeur` WHERE PhysiqueMorale=0 AND IdUtilisateur='".$_SESSION['id']."' ";
        if (!empty($_GET['q'])){
            $recherche = htmlentities($_GET['q']);
            $query = "SELECT * FROM `vendeur` WHERE  PhysiqueMorale=0 AND IdUtilisateur='".$_SESSION['id']."' AND Nom  LIKE '%".$recherche."%' ";
        };
    ...
    <div class="affichage">
        <?php foreach($data as $dataV2): ?>
                ...
                <table>
                    <td>Nom :</td>
                    <td ><?= $dataV2['Nom'] ?></td>
                    <td><a href="modification/modifier.php?id=<?=$dataV2['IdVendeur']?>">modifier</a></td>
                </table>
                <br>
        <?php endforeach;?>
    </div>
