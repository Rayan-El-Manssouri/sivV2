<?php 
        ...
        $query = "SELECT * FROM `acheteur` WHERE  PhysiqueMorale=0 AND IdUtilisateur='".$_SESSION['id']."' ";
        if (!empty($_GET['q'])){
            $recherche = htmlentities($_GET['q']);
            $query = "SELECT * FROM `acheteur` WHERE IdUtilisateur='".$_SESSION['id']."' AND Nom  AND PhysiqueMorale=0 LIKE '%".$recherche."%' ";
...
<body>
    ...
    <div class="affichage" style="position : relative; top: 250px;">
                ...
                <div style="position: relative; text-align : left; " >
                    <span >
                        ...
                        <div style="position: relative; left: 150px; top: -20px;">
                           <span><?= $dataV2['Nom'], $dataV2['RaisonSociale']; ?> <a style="position : relative; left: 20px;" href="confirmation_suprimmer/supression.php?id=<?=$dataV2['IdAcheteur']?>">suprimmer</a></span>
                        </div>
                    </span>
                </div>
                <br>
