<?php 
        ...
        $query = "SELECT * FROM `acheteur` WHERE IdUtilisateur='".$_SESSION['id']."' AND PhysiqueMorale=0 ";
        $data = $database->read($query);    
        if (!empty($_GET['q'])){
            $recherche = htmlentities($_GET['q']);
            $query = "SELECT * FROM `acheteur` WHERE IdUtilisateur='".$_SESSION['id']."' AND Nom LIKE '%".$recherche."%' AND PhysiqueMorale=0  ";
            $data = $database->read($query);
        };
        $data = $database->read($query);
        $i = 0;
